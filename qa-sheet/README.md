...
- `if`/`switch`のような構文的な分岐だけでなく、LLM instructions内のビジネスルールやenum/設定値による振る舞いの出し分けなど「コードの外にある分岐」も検出し、値の組み合わせをクロス表として洗い出す
- 大項目(対象機能)→中項目(QA観点)→小項目(個別の確認項目)の階層でTSVを出力するので、事前知識のない確認担当者に渡してもどこの話か迷わない
